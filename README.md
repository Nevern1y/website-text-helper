...
- Объединена логика воспроизведения в голосовом помощнике: добавлена единая функция остановки, которая корректно сбрасывает состояния при переключениях и размонтировании компонента.
- API-клиент дополнен методами для получения, обновления и удаления проектов, а также для работы с отдельными файлами через `/api/files/:id`.
...
5. `/image-generation` визуализирует mock-изображения, а `/voice-assistant` позволяет записывать голос, получать расшифровку и синтезировать ответ (демо WAV) без внешних интеграций.
